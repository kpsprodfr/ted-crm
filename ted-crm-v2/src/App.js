            ...
            <div id="formulaire-dropdown" style={{ position:'relative' }}>
              ...
              {showFormDropdown && (
                <div style={{ position:'absolute', top:40, left:0, background:'#fff', borderRadius:10, border:'1.5px solid #eee', boxShadow:'0 8px 24px rgba(0,0,0,0.12)', padding:8, zIndex:200, minWidth:180 }}>
                  <button onClick={async()=>{ try { await navigator.clipboard.writeText('https://ted-crm.pages.dev/reserver.html'); showToast('✅ Lien copié dans le presse-papiers !'); } catch(e) { const el=document.createElement('textarea'); el.value='https://ted-crm.pages.dev/reserver.html'; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); showToast('✅ Lien copié !'); } setShowFormDropdown(false); }} style={{ display:'block', width:'100%', textAlign:'left', padding:'9px 14px', border:'none', background:'none', cursor:'pointer', fontSize:13, fontWeight:600, borderRadius:7 }}>📋 Copier</button>
                  <button onClick={()=>{ window.open('https://ted-crm.pages.dev/reserver.html','_blank'); setShowFormDropdown(false); }} style={{ display:'block', width:'100%', textAlign:'left', padding:'9px 14px', border:'none', background:'none', cursor:'pointer', fontSize:13, fontWeight:600, borderRadius:7 }}>🔗 Ouvrir</button>
                  <button onClick={async()=>{ const url='https://ted-crm.pages.dev/reserver.html'; if(navigator.share){ try{ await navigator.share({title:'Réservation Le TED',text:'Réservez votre table au TED',url}); }catch(e){ if(e.name!=='AbortError'){ await navigator.clipboard.writeText(url); showToast('✅ Lien copié !'); } } }else{ await navigator.clipboard.writeText(url); showToast('✅ Lien copié — partagez-le comme vous voulez !'); } setShowFormDropdown(false); }} style={{ display:'block', width:'100%', textAlign:'left', padding:'9px 14px', border:'none', background:'none', cursor:'pointer', fontSize:13, fontWeight:600, borderRadius:7 }}>📤 Partager</button>
                </div>
              )}
            </div>
          </div>
              ...
              {showFormDropdown && (
                <>
                  ...
                  <div style={{ position:'absolute', top:'calc(100% + 8px)', left:0, background:'#fff', borderRadius:10, boxShadow:'0 8px 32px rgba(0,0,0,0.15)', padding:6, minWidth:200, zIndex:300 }}>
                    <button onClick={async()=>{ try{ await navigator.clipboard.writeText('https://ted-crm.pages.dev/reserver.html'); showToast('✅ Lien copié dans le presse-papiers !'); }catch(e){ const el=document.createElement('textarea'); el.value='https://ted-crm.pages.dev/reserver.html'; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); showToast('✅ Lien copié !'); } setShowFormDropdown(false); }} style={{ width:'100%', padding:'10px 14px', border:'none', background:'none', textAlign:'left', cursor:'pointer', fontSize:13, borderRadius:6, display:'flex', alignItems:'center', gap:10, color:'#111' }} onMouseEnter={e=>e.currentTarget.style.background='#f5f5f5'} onMouseLeave={e=>e.currentTarget.style.background='none'}><Copy size={15} strokeWidth={2} color="#666" /> Copier le lien</button>
                    <button onClick={()=>{ window.open('https://ted-crm.pages.dev/reserver.html','_blank'); setShowFormDropdown(false); }} style={{ width:'100%', padding:'10px 14px', border:'none', background:'none', textAlign:'left', cursor:'pointer', fontSize:13, borderRadius:6, display:'flex', alignItems:'center', gap:10, color:'#111' }} onMouseEnter={e=>e.currentTarget.style.background='#f5f5f5'} onMouseLeave={e=>e.currentTarget.style.background='none'}><ExternalLink size={15} strokeWidth={2} color="#666" /> Ouvrir</button>
                    <button onClick={async()=>{ const url='https://ted-crm.pages.dev/reserver.html'; if(navigator.share){ try{ await navigator.share({title:'Réservation Le TED',text:'Réservez votre table au TED',url}); }catch(e){ if(e.name!=='AbortError'){ await navigator.clipboard.writeText(url); showToast('✅ Lien copié !'); } } }else{ await navigator.clipboard.writeText(url); showToast('✅ Lien copié — partagez-le comme vous voulez !'); } setShowFormDropdown(false); }} style={{ width:'100%', padding:'10px 14px', border:'none', background:'none', textAlign:'left', cursor:'pointer', fontSize:13, borderRadius:6, display:'flex', alignItems:'center', gap:10, color:'#111' }} onMouseEnter={e=>e.currentTarget.style.background='#f5f5f5'} onMouseLeave={e=>e.currentTarget.style.background='none'}><Share2 size={15} strokeWidth={2} color="#666" /> Partager</button>
                  </div>
                </>
              )}
            </div>
